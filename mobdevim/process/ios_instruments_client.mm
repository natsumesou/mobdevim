...
bool print_proclist(am_device_service_connection *conn)
{
    ...
    if ( CFGetTypeID(retobj) == CFArrayGetTypeID() )
    {
        ...
        for ( NSDictionary *procDict in array )
        {
            ...
            printf(" %s%8d%s %s%s%s %s%s%s %s%s%s\n", dcolor("cyan"), _pid, colorEnd(), dcolor("gray"), startDate.description.UTF8String, colorEnd(), isApp ? dcolor("magenta") : dcolor("yellow"), _name.UTF8String, colorEnd(), dcolor("red"), path.UTF8String, colorEnd());
            //            printf(" %s%7s%s %s%s%s\n", dcolor("cyan"), [[dict[@"pid"] description] UTF8String], colorEnd(), [dict[@"isApplication"] boolValue] ? dcolor("yellow") : dcolor("bold"), [[dict[@"realAppName"] description] UTF8String], colorEnd());
        }
    }
    else
    {
